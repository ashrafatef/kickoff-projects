...
export async function askAboutGitRepo() {
  ...
  if (choice === YesNoInq.YES) {
    const git = simpleGit();
    const response: any = await Enquirer.default.prompt({
      type: 'input',
      name: 'remoteRepo',
...
async function selectSingleChoice(message: string, choices: string[]) {
  let err: any;
  let response: any;
  const select = new Enquirer.default.Select({
    name: 'choice',
    message: message,
    choices: choices,
  });
